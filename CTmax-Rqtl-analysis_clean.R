...
pdf("ltreb-only-CTmax_17-1hot-site-tanks_chr22-qtl-effect-plot-w-points.pdf",8.4,8.9)
plotPXG(data_prob,marker=mar_chr22,main='22@8.81')
dev.off()

# 2nd scan
mar_chr15 <- find.marker(data_prob, chr='ScyDAA6-5984-HRSCAF-6694', pos=5.847498)
plotPXG(data_prob,marker=mar_chr15,main='15@5.85')
effectplot(data_sim,mname1=mar_chr15,main='15@5.85')

## plot the joint effects of the chr22 QTL and chr15 peak
pdf("ltreb-only-CTmax_17-1hot-site-tanks_chr22-qtl-chr15-peak-interaction-plot.pdf",15.4,8.9)
par(mfrow=c(1,2), las=1)
effectplot(data_sim, mname1="ScyDAA6-2113-HRSCAF-2539@8.81", mname2="ScyDAA6-5984-HRSCAF-6694@5.85", main = "22@8.81 x 15@5.85")
effectplot(data_sim, mname2="ScyDAA6-2113-HRSCAF-2539@8.81", mname1="ScyDAA6-5984-HRSCAF-6694@5.85", main = "15@5.85 x 22@8.81")
dev.off()
...
## output genotypes for markers that fall under the QTL / within the 2-LOD interval: 6.963205 - 12.213851
...
lm.res<-lm(ctmax ~ geno + hi + STH.1 + STH.2 + STH.3 + STH.4 + STH.7 + STH.8 + STL.1 + 
             STL.2 + STL.4 + STL.5 + STL.6 + STL.7 + STM.2 + STM.4 + STM.5 + 
             STM.6 + STM.7,data=dp_noNA)
summary(lm.res)
# an ANOVA should give the same result
aov.res <- aov(ctmax ~ geno + hi + STH.1 + STH.2 + STH.3 + STH.4 + STH.7 + STH.8 + STL.1 + 
                 STL.2 + STL.4 + STL.5 + STL.6 + STL.7 + STM.2 + STM.4 + STM.5 + 
                 STM.6 + STM.7,data=dp_noNA)
summary(aov.res)
